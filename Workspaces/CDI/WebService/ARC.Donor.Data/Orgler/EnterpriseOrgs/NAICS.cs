...
namespace ARC.Donor.Data.Orgler.EnterpriseOrgs
{
    public class NAICS
    {
        ...
        * Purpose: This method gets all the NAICS details for an input master id */
        public IList<Entities.Orgler.EnterpriseOrgs.GetNAICSDetailsOutput> getNAICSDetails(int NoOfRecs, int PageNum, string  strEntOrgId)
        {
            ...
            crud = SQL.Orgler.EnterpriseOrgs.NAICS.getNAICSDetails(NoOfRecs, PageNum, strEntOrgId);

            //execute the search query and return back the results
            var AcctLst = rep.ExecuteStoredProcedure<Entities.Orgler.EnterpriseOrgs.GetNAICSDetailsOutput>(crud.strSPQuery, crud.parameters).ToList();
            return AcctLst;
        }
    }
}
