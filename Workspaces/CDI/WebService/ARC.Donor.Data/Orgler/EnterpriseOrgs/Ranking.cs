...
namespace ARC.Donor.Data.Orgler.EnterpriseOrgs
{
    public class Ranking
    {
        ...
        * Purpose: This method gets all the NAICS details for an input master id */
        public IList<Entities.Orgler.EnterpriseOrgs.RankingOutputModel> getRankingDetails(int NoOfRecs, int PageNum, string strEntOrgId)
        {
            ...
            crud = SQL.Orgler.EnterpriseOrgs.Ranking.getRankingSQL(NoOfRecs, PageNum, strEntOrgId);

            //execute the search query and return back the results
            var AcctLst = rep.ExecuteStoredProcedure<Entities.Orgler.EnterpriseOrgs.RankingOutputModel>(crud.strSPQuery, crud.parameters).ToList();
            return AcctLst;
        }
    }
}
