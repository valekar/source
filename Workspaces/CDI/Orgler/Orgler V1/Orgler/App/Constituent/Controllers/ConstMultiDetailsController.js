...
angular.module('constituent').controller("constMultiDetialsController", ['$scope', '$parse', '$http', '$localStorage', '$sessionStorage', '$stateParams', 'uiGridConstants', '$uibModal', '$stateParams', 'constMultiGridService',
    ...
    'constituentCRUDapiService', '$q','showDetailsPostApiService','exportService','$location','EXTENDED_CONSTANTS',
function ($scope, $parse, $http, $localStorage, $sessionStorage, $stateParams, uiGridConstants, $uibModal, $stateParams, constMultiGridService, constituentServices,
    ...
    $q, showDetailsPostApiService, exportService, $location, EXTENDED_CONSTANTS) {

    //var url = $location.absUrl();
    //alert('ConstMultiDetails: ' + url);
    ...
    var TEMPLATES = {
        ...
        RFMVALUES: 'ConstRFMValues.tpl.html',
    }
    ...
    $scope.CONSTANTS = {
        ...
        AFFILIATOR: "Affiliator", SUMMARY_VIEW: "SummaryView", MASTER_DETAIL: "MasterDetail", EMAIL_DOMAINS: "EmailDomains", NAICS_CODES: "NAICSCodes", CONTACTS_DETAILS: "ContactsDetails", ALTERNATEIDS_DETAILS: "AlternateIds", RFMVALUES_DETAILS: "RFMValues"
    };
    ...
    var initailize = function () {
        ...
        $sessionStorage.masterId = $stateParams.constituentId;     
        ...
        constituentApiService.getApiDetails('', CONSTANTS.MENU_PREF).success(function (result) {
            ...
            angular.forEach(result.Preferences, function (v, k) {
                ...
                switch (v) {
                    ...
                    case CONSTANTS.RFMVALUES_DETAILS: {
                        if ($scope.constituent_type == 'OR') {
                            $scope.constRFMValues = true;
                            $scope.getConstRFMValues(false);
                            $scope.pref_RFMValues = true;
                        }
                        break;
                    }
                    default: {
        ...
        $scope.savePreferences = function () {
            ...
            if ($scope.pref_RFMValues) {
                preferences.push(CONSTANTS.RFMVALUES_DETAILS);
            }
        ...
        /*************************************CONST RFMVALUES_DETAILS SETTINGS***********************************/
        $scope.constRFMValues = false;
        //toggle grid results
        constMultiGridService.getToggleDetails($scope, false, CONSTANTS.RFMVALUES_DETAILS);
        /*****set the grid options*****/
        var options = constMultiGridService.getGridOptions(uiGridConstants, CONSTANTS.RFMVALUES_DETAILS);
        $scope.constRFMValuesGridOptions = options;
        $scope.constRFMValuesGridOptions.onRegisterApi = function (grid) {
            $scope.constRFMValuesGridApi = grid;
        }
        /*************************************END CCONST RFMVALUES_DETAILS SETTINGS***********************************/
    ...
    //$scope.param = $stateParams.constituentId;
    //constituentApiService.getApiDetails($scope.param, CONSTANTS.BEST_SMRY).success(function (result) {
    //    //set the value in cache
    //    $rootScope.constName = result[0].name;
    //    delete $sessionStorage.name;
    //    $sessionStorage.name = $rootScope.constName;
   ...
   initailize();
    ...
    // checkbox for Contacts
    $scope.getConstRFMValues = function (constRFMValues) {

        //if ($rootScope.constituent_type == 'OR') {
        var url = COMMON_URL + TEMPLATES.RFMVALUES
        ...
        if (!constRFMValues) {
            ...
            $scope.toggleConstRFMValuesLoader = { "display": "block" };

            var finalResults = callConstSetup($scope, constituentApiService, $scope.param, uiGridConstants, constMultiDataService,
                constMultiGridService, CONSTANTS.RFMVALUES_DETAILS, $scope.constRFMValuesGridOptions, $rootScope);
            // console.log(finalResults.gridOp);
            //constituentDataServices.setconstOrgNameGrid(finalResults.gridOp);
            $scope.constRFMValuesGridOptions = finalResults.gridOp;
            $scope.currentconstRFMValuesPage = 1;

            var listener = $scope.$watch(function () {
                return $scope.constRFMValuesGridOptions.data.length;
            }, function (newLength, oldLength) {
                if (newLength > 0) {
                    $scope.totalConstRFMValuesItems = $scope.constRFMValuesGridOptions.data.length;
                    $scope.constRFMValuesCount = $scope.constRFMValuesGridOptions.data[0].distinct_count;
                    //unbind watch after use
                    listener();
                }
            });
    ...
    $scope.pageChanged = function (type, page) {
        switch (type) {
            ...
            case CONSTANTS.RFMVALUES_DETAILS: {
                $scope.constRFMValuesGridApi.pagination.seek(page);
                break;
            }
        }
    ...
    $rootScope.toggleFiltering = function (type) {
        switch (type) {
            ...
            case CONSTANTS.RFMVALUES_DETAILS: {
                $scope.constRFMValuesGridOptions.enableFiltering = !$scope.constRFMValuesGridOptions.enableFiltering;
                $scope.constRFMValuesGridApi.core.notifyDataChange(uiGridConstants.dataChange.ALL);
                break;
            }
    ...
    var extendedFuncInitialize = function () {
        ...
        $scope.multi = {
            ...
            CONSTANTS: {
                ORG_RELATIONSHIP: EXTENDED_CONSTANTS.ORG_RELATIONSHIP
            },
    ...
    $scope.multi.redirectTo = function (type) {
        ...
        switch (type) {
            case $scope.multi.CONSTANTS.ORG_RELATIONSHIP: {
                $scope.multi.orgRelationshipTemplate = $scope.multi.EXTENDED_BASE_URL + '/ConstRelationship.tpl.html';
                $scope.$broadcast('org_relationship', { 'org_relationship': $scope.multi.toggleRelationship });
        ...
        $scope.details.showDetails = function (type) {         
            switch (type) {       
                case HOME_CONSTANTS.CONST_BIRTH: {           
                    ...
                    $scope.details.title = CONSTITUENT_MESSAGES.BIRTH_TITLE;
                ...
                case HOME_CONSTANTS.CONST_DEATH: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.DEATH_TITLE;
                ...
                case HOME_CONSTANTS.CONST_EMAIL: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.EMAIL_TITLE;
                ...
                case HOME_CONSTANTS.CONST_NAME: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.IND_NAME_TITLE;
                ...
                case HOME_CONSTANTS.CONST_ORG_NAME: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.ORG_NAME_TITLE;
                ...
                case HOME_CONSTANTS.CONST_EXT_BRIDGE: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.EXT_BRID_TITLE;
                ...
                case HOME_CONSTANTS.CONST_PHONE: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.PHONE_TITLE;
                ...
                case HOME_CONSTANTS.INTERNAL_BRIDGE: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.INT_BRID_TITLE;
                ...
                case HOME_CONSTANTS.CONST_PREF: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.CNCT_PREF_TITLE;
                ...
                case HOME_CONSTANTS.CHARACTERISTICS: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.CHARAC_TITLE;
                ...
                case HOME_CONSTANTS.CONST_ADDRESS: {
                    ...
                    $scope.details.togglePleaseWait = true;
                    $scope.details.title = CONSTITUENT_MESSAGES.ADDR_TITLE;
