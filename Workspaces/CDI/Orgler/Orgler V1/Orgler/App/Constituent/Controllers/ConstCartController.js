    ...
    $scope.compare = function (type) {
        ...
        if (type === MERGE) {
            ...
            if (validate) {
                ...
                $scope.cart.pleaseWait = false;
                messagePopup($rootScope, CONSTITUENT_MESSAGES.PLEASE_SELECT, CONSTITUENT_MESSAGES.ALERT);
                return;
            }
            ...
            if (angular.isUndefined($scope.cart.mergeRows) || $scope.cart.mergeRows.length <= 0) {
                messagePopup($rootScope, CONSTITUENT_MESSAGES.ATLEAST_ONE, CONSTITUENT_MESSAGES.ALERT);
                return;
            }
