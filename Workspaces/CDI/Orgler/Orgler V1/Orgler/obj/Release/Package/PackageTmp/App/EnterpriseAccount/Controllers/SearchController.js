    ...
    function ($state, $http, $location, $rootScope, $scope, $uibModal, $localStorage, dateParser, service, dataService, uiGridConstants, uiGridTreeViewConstants
           ...
           // Set the previously searched search params if navigating from breadcrumbs
            if (SearchParams) {
                ...
                for (i = 1; i <= rootModel.length; i++) {
                    ...
                    //$scope.InputParams.SourceSystem[appendString + i] = rootModel[i - 1]["SourceSystem"];
                    //$scope.InputParams.ChapterSystem[appendString + i] = rootModel[i - 1]["ChapterSystem"];
                   // $scope.InputParams.ListNaicsCodes[appendString + i] = $rootScope.ListNaicsCode;
                    ...
                    //$scope.InputParams.Username = rootModel[i - 1]["Username"];
                    $scope.InputParams.Username[appendString + i] = rootModel[i - 1]["Username"];
                   // $scope.InputParams.RecentChanges[appendString + i] = rootModel[i - 1]["Recentchanges"];
        ...
        //When clicked in search go to search results and do a post there         
            $scope.EnterpriseAccountSearch = function (myForm) {
                if (myForm.$valid) {
                    ...
                    //if(postParams["EnterpriseOrgInputSearchModel"]!=null)
                    //callEnterpriseAccountService($scope, service, dataService, postParams, $location, NAVIGATE_URL, $rootScope);
                    //}
                    //else {
                    //    $scope.EntOrgsNoResults = "Please provide at least one input criteria before search";
                    //    $scope.showPleaseWaitText = { "display": "none" };
                    //}
                    //}
                    //else { $scope.EntOrgsNoResults = "Please enter only numbers for MasterId field" }
                }
        ...
        $scope.selectNaicsSegmentEnterpriseAccount = function (counter) {
           ...
           // $scope.showPleaseWaitText();
            ...
            // listSelectedSearchNAICSCodes = [];
            //listSelectedSearchNAICSCodes = [];
            //$rootScope.ListNaicsCodes = "";
       ...
       // $rootScope.Tags = ["Premier flag FR", "Premier flag HS", "Premier flag BIO", "Premier flag ENT", "Affiliator Visibility", "Exclude WPG", "Exclude DAF"];
        $rootScope.IncludeSuperiorOrSubordinate = ["Include Superior", "Include Subordinate"];
        //$rootScope.SourceSystemData = ["HEMO", "PHSF", "SABO", "TAFS", "SFFS", "RCSO","EOSI"];
        //$rootScope.RankProvider = ["Fortune - 2019", "Fortune - 2018", "Fortune - 2017", "Fortune - 2016", "Fortune - 2015", "Fortune - 2014", "Fortune - 2013", "Fortune - 2012", "Fortune - 2011", "Fortune - 2010"];
...
enterpriseAccMod.filter('placeholderfunc', ['EnterpriseAccountDataService', function (EnterpriseAccountDataService) {
    return function (input) {
        ...
        angular.forEach(input.EnterpriseOrgInputSearchModel, function (value, key) {
            ...
            //    EnterpriseAccountResultsBindingData += "Recent Changes: " + value.RecentChanges + ", ";
