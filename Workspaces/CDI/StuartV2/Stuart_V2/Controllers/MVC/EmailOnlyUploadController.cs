...
namespace Stuart_V2.Controllers.MVC
{
    ...
    public class EmailOnlyNativeController : BaseController
    {
        ...
        public async Task<JsonResult> postEmailOnlyUploadData(string uploadedEmailFormData)
        {
            ...
            return processResultForUploadInsertion(res, "Email-Only Upload");
        }
            ...
            for (int iCnt = 0; iCnt <= hfc.Count - 1; iCnt++)
            {
                ...
                if (hpf.ContentLength > 0)
                {
                    ...
                    if (UploadFlag)    // If the file is not a duplicate, perform below actions
                    {
                                    ...
                                    try
                                    {
                                        ...
                                        if (check)
                                        {
                                            ...
                                            for (int rw = 3; rw <= ws.Dimension.End.Row; rw++)
                                            {
                                                ...
                                                {
                                                    EmailOnlyUploadInput input = new EmailOnlyUploadInput()
                                                    {
                                                    ...
                                                    lgl.EmailOnlyUploadInputList.Add(input);
                                                }
                                            }
                                        }
                                        else
                        ...
                        if (lgl.EmailOnlyUploadInputList.Count == 0)
                        {
                            var result = new JsonResult();
                            result.Data = "EmptyFileUploaded"; // Return a FileAlread
                            result.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
