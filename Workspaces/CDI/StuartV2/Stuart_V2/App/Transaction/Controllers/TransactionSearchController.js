    ...
    var initialize = function () {
        ...
        if (SearchParams) {
            ...
            for (i = 1; i <= rootModel.length; i++) {
               ...
               // $scope.TransParams.transactionDateFrom[appendString + i] = new Date(rootModel[i - 1]["FromDate"]);
                ...
                $scope.TransParams.showCloseButton[appendString + i] = true;
                if (rootModel[i - 1]["FromDate"]) {
                    $scope.TransParams.transactionDateFrom[appendString + i] = new Date(rootModel[i - 1]["FromDate"]);
                }
                else {
                    $scope.TransParams.transactionDateFrom[appendString + i] = null;
                }
                if (rootModel[i - 1]["ToDate"]) {
                    $scope.TransParams.transactionDateTo[appendString + i] = new Date(rootModel[i - 1]["ToDate"]);
                }
                else {
